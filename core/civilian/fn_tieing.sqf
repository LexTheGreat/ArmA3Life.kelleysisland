...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["tied",FALSE]) && vehicle player == player) exitWith {
			player setVariable["restrainedZip",FALSE,TRUE];
...
if((player getVariable["surrender",FALSE])) then { player setVariable["surrender",FALSE,TRUE]; player playMove ""; };	
...
{
	player playMove "AmovPercMstpSlowWrflDnon_SaluteIn";
	player setVariable ["restrainedZip",false,true];
	player setVariable ["Escorting",false,true];
	player setVariable ["transporting",false,true];
	player setVariable ["tied",false,true];;
