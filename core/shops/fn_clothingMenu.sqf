...
if((_this select 3) in ["cop","brucecop","dtu","swatcop","copsp"] && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) in ["doj"] && __GETC__(life_dojlevel) < 1) exitWith {hint localize "STR_Shop_Notadoj"; closeDialog 0;};
if((_this select 3) in ["copms"] && __GETC__(life_dojlevel) < 5) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
