...
{
	private["_hideout","_gangleader"];
	_hideout = (nearestObjects[getPosATL player,["Land_u_Barracks_V2_F","Land_i_Barracks_V2_F","Land_Barn_W_01","Land_Metal_Shed_F"],25]) select 0;
	_gangleader = player getvariable "gangleader";
	...
	if ((getPlayerUID player) == "_gangleader") then {
		...
		if (typeOf cursortarget IN ["A3L_GMC_Van_Food","A3L_GMC_Van_General","A3L_GMC_Van_Clothing"] && count crew cursortarget != 0) then {
			mobile_payment = _price * 0.8;
			_unit = (driver cursortarget);
			_price = _price * 0.85;
