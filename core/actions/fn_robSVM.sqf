...
[[1,format["911 DISPATCH: Smallville Market has triggered an emergency alarm!",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
while {true} do {
		...
		if (!alive _robber) exitwith
			{
				[[1,format["911 DISPATCH: ONE ASSAILANT HAS DIED: Smallville Market",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
				hint "STORE CLERK: Oh damn, you just got blood on my cash register.";
					sleep 450;
				_action = _shop addAction["Rob Store",life_fnc_robSVM];
				};
			if (_dist >= 6) exitwith {
			[[1,format["911 DISPATCH: ONE ASSAILANT HAS FLED: Smallville Market",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
[[1,format["Lakeside NEWS: Smallville Market was just hit for $%2", _shop, [_funds] call life_fnc_numberText]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
_funds = 0;
life_use_atm = false;
sleep 180;
life_use_atm = true;
