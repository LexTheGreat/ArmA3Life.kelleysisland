...
[1,format["911 DISPATCH: Van Der Ville General Store has person(s) with weapons!",_storename]] remoteExec ["life_fnc_broadcast",west];
...
while {true} do {
		...
		if (!alive _robber) exitwith
			{
[1,format["911 DISPATCH: ONE ASSAILANT HAS DIED: Van Der Ville General Store",_storename]] remoteExec ["life_fnc_broadcast",west];
				hint "STORE CLERK: Oh damn, you just got blood on my cash register.";
				sleep 300;
				_action = _shop addAction["Rob Store",life_fnc_robShops];
			};
			if (_dist >= 6) exitwith {
[1,format["911 DISPATCH: ONE ASSAILANT HAS FLED: Van Der Ville General Store",_storename]] remoteExec ["life_fnc_broadcast",west];
...
[1,format["Erie County NEWS: Van Der Ville General Store just got robbed for $%2", _shop, [_funds] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",civilian];
_funds = 0;
life_use_atm = false;
sleep 180;
life_use_atm = true;
