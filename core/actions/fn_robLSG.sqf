		...
		if (!alive _robber) exitwith
			{
				...
				sleep 300;
				_action = _shop addAction["Rob Store",life_fnc_robLSG];
			};
			if (_dist >= 6) exitwith {
...
[[1,format["Lakeside NEWS: Lakeside City General Store just got robbed for $%2", _shop, [_funds] call life_fnc_numberText]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
_funds = 0;
life_use_atm = false;
sleep 180;
life_use_atm = true;
sleep 900;//Cooldown between the robberies
_action = _shop addAction["Rob Store",life_fnc_robLSG];//Adding action for the robbery
};
