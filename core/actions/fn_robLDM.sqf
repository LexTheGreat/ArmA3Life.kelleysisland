...
[[1,format["911 DISPATCH: Los Diablos Market has triggered an emergency alarm!",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		...
		if (!alive _robber) exitwith
			{
				...
				hint "STORE CLERK: Oh damn, you just got blood on my cash register.";
					sleep 600;
				_action = _shop addAction["Rob Store",life_fnc_robLDM];
				};
			if (_dist >= 6) exitwith {
...
[[1,format["Lakeside NEWS: Los Diablos Market was just hit for $%2", _shop, [_funds] call life_fnc_numberText]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
_funds = 0;
life_use_atm = false;
sleep 180;
life_use_atm = true;
