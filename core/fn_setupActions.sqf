...
' !isNull nearestObject (screenToWorld [0.5,0.5]) && count (nearestObject (screenToWorld [0.5,0.5]) getVariable ["item",[]]) > 0 && player distance nearestObject (screenToWorld [0.5,0.5]) < 5 && !life_action_inUse ']];
		...
		!isNull cursorTarget && player distance cursorTarget < 3.5 && isPlayer cursorTarget && animationState cursorTarget in ["Unconscious","amovpercmstpsnonwnondnon_amovpercmstpssurwnondnon"] && !(cursorTarget getVariable["robbed",FALSE]) ']];
		
		//Tie person
		life_actions = life_actions + [player addAction["Tie Person",life_fnc_tieingAction,"",0,false,false,"",'
		!isNull cursorTarget && player distance cursorTarget < 2.5 && isPlayer cursorTarget && (animationState cursorTarget == "Unconscious" || animationState cursorTarget == "AmovPercMstpSsurWnonDnon" && (!life_action_inUse) ) ']];
		
		// Suicide Bomb 
        life_actions = life_actions + [player addAction["Activate Suicide Jacket",life_fnc_suicideBomb,"",0,false,false,"",
        'vest player == "A3L_SuicideVest" && alive player && playerSide == civilian && !life_isSuiciding && !life_isSuicide']];
	};
	case west:
	{
	//Untie person
